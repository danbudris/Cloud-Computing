...
# map the values to a list (for appending to) and then reduce by key, appending values together
allDictionaryWordsInEachDoc = justDocAndPos.map(lambda x: (x[0], [x[1]])).reduceByKey(lambda a, b: a + b)
...
# use list comprehension to convert to 0/1 and then convert to a numpy array
# this hosed my CPU on my laptop, but seemed to have worked
zeroOrOne = allDocsAsNumpyArrays.map (lambda x: (x[0], np.array([ 1 if elem  > 0 else 0 for elem in x[1] ])))

# Now, add up all of those arrays into a single array, where the 
# i^th entry tells us how many
# individual documents the i^th word in the dictionary appeared in
# this also hosed my laptop; 100% CPU on all 8 cores for more than a minute
...
# take an array of shape 20k filled with values equal to the number of documents (multipier) and divide it by the dfArray
# returns an array of shape 20k in one dimension filled with number of documents/ documents that the ith word appears in: the idf!
...
def getPrediction (textInput, k):
    ...
    distances = allDocsAsNumpyArrays.map (lambda x: (x[0], (np.dot(x[1], myArray)/(np.linalg.norm(x[1])*np.linalg.norm(myArray)))) )
    #
